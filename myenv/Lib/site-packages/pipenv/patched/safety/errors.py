from pipenv.patched.safety.constants import (
    EXIT_CODE_FAILURE,
...
class DatabaseFileNotFoundError(DatabaseFetchError):
    ...
    def __init__(
        self, db=None, message="Unable to find vulnerability database in {db}"
    ):
        self.db = db
...
class InvalidKeyError(DatabaseFetchError):
    ...
    def __init__(
        self,
        key=None,
        message="Your API Key '{key}' is invalid. See {link}.",
        reason=None,
    ):
        self.key = key
...
class TooManyRequestsError(DatabaseFetchError):
    ...
    def __init__(self, reason=None, message="Too many requests."):
        info = f" Reason: {reason}"
...
class NetworkConnectionError(DatabaseFetchError):
    ...
    def __init__(
        self, message="Check your network connection, unable to reach the server."
    ):
        self.message = message
        super().__init__(self.message)
