...
from pipenv.patched.safety.output_utils import (
...
"""
    ...
    def __build_announcements_section(self, announcements):
        ...
        if basic_announcements:
            announcements_content = build_announcements_section_content(
                basic_announcements
            )
            announcements_section = [
                add_empty_line(),
                self.ANNOUNCEMENTS_HEADING,
                add_empty_line(),
    ...
    def render_vulnerabilities(
        self, announcements, vulnerabilities, remediations, full, packages
    ):
        ...
        if primary_announcement:
            end_content = [
                add_empty_line(),
                build_primary_announcement(
                    primary_announcement, columns=get_terminal_size().columns
                ),
                self.DIVIDER_SECTIONS,
            ]
        ...
        report_brief_section = build_report_brief_section(
        ...
        if vulnerabilities:
            ...
            final_brief = get_final_brief(
                len(vulnerabilities), len(remediations), ignored, total_ignored
            )

            return "\n".join(
                [ScreenReport.REPORT_BANNER]
                + announcements_section
                + [
                    ...
                    self.DIVIDER_SECTIONS,
                    format_long_text(
                        click.style("VULNERABILITIES FOUND", bold=True, fg="red")
                    ),
                    self.DIVIDER_SECTIONS,
                    add_empty_line(),
                    "\n\n".join(table),
            ...
            content = format_long_text(
                click.style(
                    "No known security vulnerabilities found.", bold=True, fg="green"
                )
            )
            return "\n".join(
                [ScreenReport.REPORT_BANNER]
    ...
    def render_licenses(self, announcements, licenses):
        ...
        if not licenses:
            content = format_long_text(
                click.style("No packages licenses found.", bold=True, fg="red")
            )
            return "\n".join(
                [ScreenReport.REPORT_BANNER]
                + announcements_section
