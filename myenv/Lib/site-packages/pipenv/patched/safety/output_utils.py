...
from pipenv.patched.safety.util import (
    get_safety_version,
    Package,
    get_terminal_size,
    SafetyContext,
...
def build_announcements_section_content(
    announcements,
...
def style_lines(
    lines, columns, pre_processed_text="", start_line=" " * 4, end_line=" " * 4
):
    ...
    for line in lines:
        ...
        styled_text += (
            format_long_text(
                styled_line,
                columns=columns,
                start_line_decorator=start_line,
...
def format_vulnerability(
    vulnerability, full_mode, only_text=False, columns=get_terminal_size().columns
):

    common_format = {
        "left_padding": 3,
        "format": {"sub_indent": " " * 3, "max_lines": None},
    }

    styled_vulnerability = [
        {
            "words": [
                {"style": {"bold": True}, "value": "Vulnerability ID: "},
    ...
    vulnerability_spec = [
        {
            "words": [
                {"style": {"bold": True}, "value": "Affected spec: "},
                {"value": vulnerability.vulnerable_spec},
            ]
        }
    ]
    ...
    if cve:
        ...
        if cve.cvssv3 and "base_severity" in cve.cvssv3.keys():
            ...
            cve_lines = [
                {
                    "words": [
                        {"style": {"bold": True}, "value": "{0} is ".format(cve.name)},
                        {
    ...
    if not vulnerability.ignored:
        ...
        if vulnerability.ignored_expires:
            generic_reason += (
                f" until {vulnerability.ignored_expires.strftime('%Y-%m-%d %H:%M:%S UTC')}. "
        ...
        if vulnerability.ignored_reason:
            specific_reason = [
                {
                    "words": [
                        {"style": {"bold": True}, "value": "Reason: "},
                        {"value": vulnerability.ignored_reason},
                    ]
                }
            ]
...
def format_license(license, only_text=False, columns=get_terminal_size().columns):
    to_print = [
        {
            "words": [
                {"style": {"bold": True}, "value": license["package"]},
                {
                    "value": " version {0} found using license ".format(
                        license["version"]
                    )
                },
...
def build_remediation_section(
    remediations, only_text=False, columns=get_terminal_size().columns, kwargs=None
):
    columns -= 2
    left_padding = " " * 3
    ...
    for pkg in remediations.keys():
        ...
        other_options = [
            str(fix)
            for fix in remediations[pkg].get("secure_versions", [])
            if str(fix) != fix_version
        ]
        ...
        if other_options:
            ...
            raw_recommendation = (
                f"{raw_recommendation} {raw_pre_other_options} " f"{raw_other_options}"
            )

        remediation_content = [
            f"{left_padding}The closest version with no known vulnerabilities is "
            + click.style(upgrade_to, bold=True),
            new_line,
        ...
        if not fix_version:
            remediation_content = [
                new_line,
                click.style(
                    f"{left_padding}There is no known fix for this vulnerability.",
                    bold=True,
                    fg="yellow",
                ),
            ]
        ...
        text = (
            "vulnerabilities"
            if remediations[pkg]["vulns_found"] > 1
            else "vulnerability"
        )

        raw_rem_title = (
            f"-> {pkg} version {remediations[pkg]['version']} was found, "
    ...
    title = format_long_text(
        click.style(f"{left_padding}REMEDIATIONS", fg="green", bold=True), **kwargs
    )
    ...
    if not is_using_api_key():
        ...
        msg = (
            "{0} {1} found in {2} {3}. "
            "For detailed remediation & fix recommendations, upgrade to a commercial license.".format(
                total_vulns, vuln_text, total_packages, pkg_text
            )
        )
...
def get_final_brief(
    total_vulns_found, total_remediations, ignored, total_ignored, kwargs=None
):
    if not kwargs:
        kwargs = {}
    ...
    ignored_text = (
        f" {total_ignored} {vuln_text} from {len(ignored.keys())} {pkg_text} ignored."
        if ignored
        else ""
    )
    remediation_text = (
        f" {total_remediations} remediation{' was' if total_remediations == 1 else 's were'} "
        f"recommended."
        if is_using_api_key()
        else ""
    )
...
def format_long_text(
    ...
    for line in lines:
        ...
        for wrapped_line in wrapped_lines:
            ...
            formatted_lines.append(
                f"{start_line_decorator}{new_line}{end_line_decorator}"
            )
...
def get_printable_list_of_scanned_items(scanning_target):
    ...
    if scanning_target == "environment":
        locations = set(
            [pkg.found for pkg in context.packages if isinstance(pkg, Package)]
        )
...
def build_report_brief_section(
    columns=None, primary_announcement=None, report_type=1, **kwargs
):
    if not columns:
        columns = get_terminal_size().columns
    ...
    if primary_announcement:
        styled_brief_lines.append(
            build_primary_announcement(
                columns=columns, primary_announcement=primary_announcement
            )
        )
...
def build_report_for_review_vuln_report(as_dict=False):
    ...
    using_sentence = build_using_sentence(
    ...
    old_timestamp = [
        {"style": False, "value": "Report generated "},
        {"style": True, "value": old_timestamp},
    ]
    now = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    current_timestamp = [
        {"style": False, "value": "Timestamp "},
        {"style": True, "value": now},
    ]

    brief_info = (
        [
            [
                {"style": False, "value": "Safety "},
                {
                    "style": True,
                    "value": "v" + report_from_file.get("safety_version", "-"),
                },
...
def build_using_sentence(key, db):
    key_sentence = []
    custom_integration = (
        os.environ.get("SAFETY_CUSTOM_INTEGRATION", "false").lower() == "true"
    )

    if key:
        key_sentence = [
            {"style": True, "value": "an API KEY"},
...
def build_scanned_count_sentence(packages):
    ...
    if len(packages) >= 1:
        scanned_count = "Found and scanned {0} {1}".format(
            len(packages), "packages" if len(packages) > 1 else "package"
        )
...
def get_report_brief_info(as_dict=False, report_type=1, **kwargs):
    ...
    scanning_types = {
        "check": {
            ...
            "scanning_target": "environment",
        },  # Files, Env or Stdin
        "license": {
            "name": "Licenses",
            "action": "Scanning licenses",
            "scanning_target": "environment",
        },  # Files or Env
        "review": {
            "name": "Report",
            "action": "Reading the report",
    ...
    action_executed = [
        {
            "style": True,
            "value": scanning_types.get(context.command, {}).get("action", ""),
        },
        {"style": False, "value": " in your "},
    ...
    brief_data["policy_file"] = (
        policy_file.get("filename", "-") if policy_file else None
    )
    brief_data["policy_file_source"] = (
        "server"
        if brief_data["policy_file"]
        and "server-safety-policy" in brief_data["policy_file"]
        else "local"
    )
    ...
    if context.params.get("audit_and_monitor"):
        logged_url = (
            context.params.get("audit_and_monitor_url")
            if context.params.get("audit_and_monitor_url")
            else "https://pyup.io"
        )
        audit_and_monitor = [
    ...
    if report_type == 1:
        ...
        additional_data = [
            [
                ...
                {
                    "style": True,
                    "value": f' vulnerabilit{"y" if brief_data["vulnerabilities_found"] == 1 else "ies"} found',
                },
            ],
            [
                ...
                {
                    "style": True,
                    "value": f' vulnerabilit{"y" if brief_data["vulnerabilities_ignored"] == 1 else "ies"} ignored',
                },
            ],
        ]

        if is_using_api_key():
            brief_data["remediations_recommended"] = kwargs.get(
            ...
            additional_data.extend(
                [
                    [
                        ...
                        {
                            "style": True,
                            "value": f' remediation{"" if brief_data["remediations_recommended"] == 1 else "s"} recommended',
                        },
                    ]
                ]
            )
        ...
        additional_data = [
            [
                ...
                {
                    "style": True,
                    "value": f' license {"type" if brief_data["licenses_found"] == 1 else "types"} found',
                },
            ],
        ]
    ...
    using_sentence_section = (
        [nl] if not using_sentence else [nl] + [build_using_sentence(key, db)]
    )
    scanned_count_sentence = build_scanned_count_sentence(packages)

    timestamp = [
        {"style": False, "value": "Timestamp "},
        {"style": True, "value": current_time},
    ]
