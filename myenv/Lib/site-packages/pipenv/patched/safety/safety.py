...
def write_to_cache(db_name, data):
    ...
    with open(CACHE_FILE, "r") as f:
        try:
            ...
            LOG.debug(
                "JSONDecodeError in the local cache, dumping the full cache file."
            )
            cache = {}
...
def fetch_policy(key, proxy):
    ...
    try:
        ...
        r = session.get(
            url=url, timeout=REQUEST_TIMEOUT, headers=headers, proxies=proxy
        )
        LOG.debug(r.text)
        return r.json()
    except:
        ...
        click.secho(
            "Warning: couldn't fetch policy from pyup.io.", fg="yellow", file=sys.stderr
        )
...
def post_results(key, proxy, safety_json, policy_file):
    ...
    audit_report = {"safety_json": json.loads(safety_json), "policy_file": policy_file}
    ...
    try:
        ...
        click.secho(
            "Warning: couldn't upload results to pyup.io.", fg="yellow", file=sys.stderr
        )
...
def fetch_database(
    full=False, key=False, db=False, cached=0, proxy=None, telemetry=True
):
    if key:
        mirrors = API_MIRRORS
    elif db:
...
def ignore_vuln_if_needed(vuln_id, cve, ignore_vulns, ignore_severity_rules):
    ...
    ignore_severity_below = float(
        ignore_severity_rules.get("ignore-cvss-severity-below", 0.0)
    )
    ignore_unknown_severity = bool(
        ignore_severity_rules.get("ignore-cvss-unknown-severity", False)
    )
...
def check(
    ...
    db = fetch_database(
        key=key, db=db_mirror, cached=cached, proxy=proxy, telemetry=telemetry
    )
    db_full = None
                ...
                if spec_set.contains(pkg.version):
                    ...
                    for data in get_vulnerabilities(
                        ...
                        ignore_vuln_if_needed(
                            vuln_id, cve, ignore_vulns, ignore_severity_rules
                        )
...
def precompute_remediations(remediations, package_metadata, vulns, ignored_vulns):
    for vuln in vulns:
        if vuln.ignored:
        ...
        if vuln.package_name in remediations.keys():
            remediations[vuln.package_name]["vulns_found"] = (
                remediations[vuln.package_name].get("vulns_found", 0) + 1
            )
        else:
            vulns_count = 1
            package_metadata[vuln.package_name] = {
                "insecure_versions": vuln.pkg.insecure_versions,
                "secure_versions": vuln.pkg.secure_versions,
                "version": vuln.pkg.version,
            }
    ...
    for pkg_name in remediations.keys():
        ...
        if not ignored_vulns:
            ...
            secure_v = compute_sec_ver_for_user(
                package=pkg_name, ignored_vulns=ignored_vulns, db_full=db_full
            )

        remediations[pkg_name]["secure_versions"] = secure_v
        remediations[pkg_name]["closest_secure_version"] = get_closest_ver(
            secure_v, pkg.get("version", None)
        )
...
def review(report=None, params=None):
    ...
    pkgs = {
        pkg_name: Package(**pkg_values) for pkg_name, pkg_values in packages.items()
    }
    ctx = SafetyContext()
