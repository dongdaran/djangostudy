...
class DependentOption(click.Option):
    ...
    def handle_parse_result(self, ctx, opts, args):
        missing_required_arguments = (
            self.required_options.difference(opts) and self.name in opts
        )
...
def transform_ignore(ctx, param, value):
    if isinstance(value, tuple):
        return dict(
            zip(value, [{"reason": "", "expires": None} for _ in range(len(value))])
        )
...
class SafetyPolicyFile(click.ParamType):
    ...
    def fail_if_unrecognized_keys(
        self, used_keys, valid_keys, param=None, ctx=None, msg="{hint}", context_hint=""
    ):
    ...
    def shell_complete(self, ctx: "Context", param: "Parameter", incomplete: str):
