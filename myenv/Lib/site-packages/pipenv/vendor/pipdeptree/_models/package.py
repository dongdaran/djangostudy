...
class DistPackage(Package):
    ...
    def render_as_root(self, *, frozen: bool) -> str:
        return (
            self.as_frozen_repr(self._obj)
            if frozen
            else f"{self.project_name}=={self.version}"
        )
...
class ReqPackage(Package):
    ...
    def version_spec(self) -> str | None:
        result = None
        specs = sorted(
            map(str, self._obj.specifier), reverse=True
    ...
    def is_conflicting(self) -> bool:
        ...
        return not self._obj.specifier.contains(
            self.installed_version, prereleases=True
        )
    ...
    def as_dict(self) -> dict[str, str]:
        return {
            ...
            "required_version": (
                self.version_spec if self.version_spec is not None else "Any"
            ),
        }
